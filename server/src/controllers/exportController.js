...
// 4. Xuất phiếu thu đi đòi nợ (Để in ấn)
...
// 5. Xuất hóa đơn riêng lẻ (của hộ sau khi đóng tiền)
const exportOneReceipt = async (req, res) => {
  try {
    const { paymentId } = req.params;
    const buffer = await exportService.exportSingleReceipt(paymentId);

    // Đặt tên file
    const fileName = `PhieuThu_${paymentId}.xlsx`;
    ...
    return res.status(500).json({
      success: false,
      message: "Lỗi in hóa đơn: " + error.message,
    });
  }
};
