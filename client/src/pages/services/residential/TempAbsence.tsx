...
export default function TempAbsence() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.contactPhone && !/^[0-9]{10,11}$/.test(formData.contactPhone)) {
      newErrors.contactPhone = "Số điện thoại phải có 10-11 chữ số";
    }
      ...
      >
        ...
        <div className="space-y-4 border-t border-second/20 dark:border-second/30 pt-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                onChange={(value) => handleInputChange("tempAbsenceFromDate", value)}
              ...
              <FormField
                ...
                onChange={(value) => handleInputChange("tempAbsenceToDate", value)}
                error={errors.tempAbsenceToDate}
                hideLabel
              />
            </div>
            {errors.tempAbsenceFromDate && (
              <p className="text-xs text-red-500">{errors.tempAbsenceFromDate}</p>
            )}
            {errors.tempAbsenceToDate && (
              <p className="text-xs text-red-500">{errors.tempAbsenceToDate}</p>
            )}
          </div>
          ...
          <FormField
            ...
            placeholder="Nhập lý do tạm vắng (không bắt buộc)"
            rows={3}
          />

          {/* Điện thoại liên hệ */}
          <FormField
            label="Điện thoại liên hệ (nếu có)"
            required={false}
            type="text"
